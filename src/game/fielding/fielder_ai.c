...
static inline f32 distanceXZ(f32 x1, f32 z1, f32 x2, f32 z2) {
    f32 dx = x1 - x2;
    f32 dz = z1 - z2;
    dx = dx * dx;
    dz = dz * dz;
    return dolsqrtf2(dx + dz);
}
...
static inline BOOL fn_3_A295C_inline(void) {
    ...
    if (pct > 0.2f && fielderDesperationLevel[originalBaseOfRunnerInThisBaseline[2]] < 3 &&
        g_Ball.ballZoneAwayFromHome <= 1 && pct <= 0.25f &&
        !(pct >= 0.25f && runner->runningDirectionCode == 1) && (hexBaserunnerTracker & 1) != 0 &&
        g_Ball.ballAngleFromHome > 0x1C0 && g_Ball.ballAngleFromHome < 0x300 && pct <= 0.3f &&
...
static inline BOOL fn_3_A2FD8_inline(void) {
    ...
    if (urgencyLevel <= 1 && g_Ball.ballDistanceFromBase[3] < 5.0f &&
...
static inline int throwFramesForDistance(f32 dist, f32 speed) {
    int frames;
    ...
    if (dist > 60.0f) {
        frames = 1.15f * dist / speed;
    } else if (dist > 30.0f) {
        frames = dist / speed;
    } else {
        frames = 0.8f * dist / speed;
    }
    if (dist > 80.0f) {
        frames *= 1.3f;
        frames += 0x1e;
...
static inline int throwFramesToPoint(InMemFielder* thrower, f32 x, f32 z) {
    f32 speed = thrower->modifiedThrowingArm / 200.0f;
    f32 dx = x - thrower->pos.x;
    f32 dz = z - thrower->pos.z;
    f32 dist;
    int frames;
...
void makeThrowVariables(int fielderIndex) {
    ...
    ballX = fielder->pos.x + g_Ball.offsetWhilePickedUpHistory[0].x;
    ballY = fielder->pos.y + g_Ball.offsetWhilePickedUpHistory[0].y;
    g_FieldingLogic.IsChemistryThrow = 0;
    ballZ = fielder->pos.z + g_Ball.offsetWhilePickedUpHistory[0].z;
    g_Ball.IsAntichemistryThrow = 0;
    g_Ball.AtBat_Contact_BallPos.x = ballX;
    g_Ball.AtBat_Contact_BallPos.y = ballY;
    g_Ball.AtBat_Contact_BallPos.z = ballZ;
    ...
    if ((s16)loc >= 0 && loc <= 3) {
        ...
        if (chem < chemThresholds[1]) {
            ...
            if (!ACTIVE_TUTORIAL() && rng < prob) {
                ...
                sqx = xDist * xDist;
                dist = dolsqrtf2(sqx + zDist * zDist);
        ...
        speed = thrower->modifiedThrowingArm / 200.0f;
        dx = g_Fielders[at4].pos.x - thrower->pos.x;
        dz = g_Fielders[at4].pos.z - thrower->pos.z;
        dx = dx * dx;
        dz = dz * dz;
        dist = dolsqrtf2(dx + dz);
        if (0.0f == speed) {
        ...
        frames -= 10;
        if (g_Fielders[at4].numFramesToGetToAutoLocation < frames) {
            targetX = g_Fielders[at4].IntendedLocation.x;
            targetZ = g_Fielders[at4].IntendedLocation.z;
        } else {
            targetX = frames * g_Fielders[at4].velocityX + g_Fielders[at4].pos.x;
            targetZ = frames * g_Fielders[at4].velocityZ + g_Fielders[at4].pos.z;
        }
        g_Ball.fielderBeingThrownTo = at4;
    }
    g_Ball.throwTarget.x = targetX;
    g_Ball.throwTarget.z = targetZ;
    armSpeed = (u8)getThrowSpeedBasedOnArmStrengthStat(fielder->throwingArm);
...
void initializeThrowAngle_Speed_Length(int framesForFielderToGetToLoc, f32* throwAngle, f32* throwSpeed) {
    ...
    speed = *throwSpeed;
    pastReceiver = -9999.9f;
    yType = 0;
    reduceGravity = 0.0f;
    airFactor = 1.0f - (f32)g_Ball.airResistance / 10000.0f;
    prevYType = 0;
    destY = 0.2f;
    ...
    receiverY = g_Fielders[g_FieldingLogic.fielderAssignedLocationIndex[g_FieldingLogic.locationThrownTo]].hitbox[3] -
                0.2f;
    if (0.2f < 0.5f * receiverY) {
        destY = 0.5f * receiverY;
    }
    dx = g_Ball.throwTarget.x - g_Ball.AtBat_Contact_BallPos.x;
    dz = g_Ball.throwTarget.z - g_Ball.AtBat_Contact_BallPos.z;
    sqx = dx * dx;
    sqz = dz * dz;
    targetDist = dolsqrtf2(sqx + sqz);
    angle = 0.0f;
    bestAngle = angle;
    angleDelta = 0.025f;
    ...
    do {
        ...
        while (frames < 900) {
            frames++;
            groundSpeed *= airFactor;
            actualDist += groundSpeed;
    ...
    if (yType == 1) {
        ...
        if (actualDist > 14.0f && actualDist < 20.0f && g_FieldingLogic.locationThrownTo == 0 &&
...
void setCutoffThrowTargetLocation(f32* targetX, f32* targetZ) {
    ...
    cutoff = &g_Fielders[g_FieldingLogic.cutoffFielderIndex];
    thrower = &g_Fielders[g_Ball.fielderWBallIndex];
    frames = throwFramesToPoint(thrower, cutoff->pos.x, cutoff->pos.z);
...
BOOL fn_3_AABF8(void) {
    ...
    return !(g_Fielders[g_Ball.fielderWBallIndex].distanceToMound < 10.0f);
}
...
void fn_3_AAA3C(int fielderIdx) {
    ...
    if (g_FieldingLogic.xcc_Stored_0 >= 0 && g_FieldingLogic.locationThrownTo >= 0 &&
        ...
        (!(fielder->distanceToBases[g_FieldingLogic.locationThrownTo] > 10.0f) ||
        ...
        humanInput:
            ...
            afterChase:
                ...
                if (g_FieldingLogic.locationThrownTo >= 0 && g_FieldingLogic.locationThrownTo <= 3) {
                    ...
                    g_Ball.throwDestination.y = 0.0f;
                    ...
                    if (mound == -1) {
                        ...
                        pitcherCutoffPossible = 0;
                    } else if (g_Fielders[g_Ball.fielderWBallIndex].distanceToMound < 10.0f) {
                        pitcherCutoffPossible = 0;
                    } else {
...
void fielderKnockback(int fielderIndex) {
    ...
    if (fielder->timeSinceThrowWasCaught == 0) {
        ...
        fielder->knockBackRelatedVeloX = 0.7f * knock.x;
        fielder->knockbackRelatedVeloZ = 0.7f * knock.z;
        fielder->_0200 = 1;
    ...
    vx = vx * vx;
    sqz = vz * vz;
    fielder->currentVelocity = dolsqrtf2(vx + sqz);
    if (updateFielderPosition_checkFielderCollision(fielderIndex, &knock) != 0) {
        fielder->knockBackRelatedVeloX = 0.0f;
        fielder->knockbackRelatedVeloZ = 0.0f;
...
void knockBallLoose(int fielderIndex, int knockOutCause) {
    ...
    if (g_Ball.fielderWBallIndex == fielderIndex) {
        ...
        g_Ball.physicsSubstruct.velocity.y = 0.0001f * (f32)RandomInt_Game(300) + 0.02f;
        groundVelocity = 0.0001f * (f32)RandomInt_Game(200) + 0.05f;
...
void fn_3_A89D4(void) {
    ...
    if (g_FieldingLogic.locationThrownTo >= 0 && g_FieldingLogic.locationThrownTo <= 3) {
        if (fielder->always0_ == 1 && g_Ball.timeSinceBallPickedUp < 0x2d) {
            dx = 2.0f * fielder->xMovementDir + fielder->pos.x - g_Ball.throwDestination.x;
            dz = 2.0f * fielder->zMovementDir + fielder->pos.z - g_Ball.throwDestination.z;
            sqx = dx * dx;
            sqz = dz * dz;
    ...
    if (zone <= 2) {
        for (i = 0; i < 4; i++) {
            if (g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
                (g_Runners[i].forceOutCd == FORCE_OUT_TYPE_FORCED_TO_ADVANCE ||
                 g_Runners[i].tagUpInd == TAG_UP_TYPE_TAGGED || g_Runners[i].overrunBaseStage >= 3 ||
                 (g_Runners[i].percentTowardsNextBase <= 0.15f && g_Runners[i].runningDirectionCode == 1) ||
                 (g_Runners[i].percentTowardsNextBase >= 0.15f &&
                  g_Runners[i].percentTowardsNextBase <= 0.8f))) {
                break;
            }
        }
        if (i >= 4) {
    ...
    if (zone >= 3) {
        if (contact != BALL_RESULT_TYPE_CAUGHT && g_Runners[0].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
            g_Runners[0].fractionalBasesRan < 1.0f && g_FieldingLogic.locationThrownTo == 1) {
            g_FieldingLogic.throwSpeedType = 3;
        }
        return;
    }
...
void maybeUnused_SetThrowSpeedType2(f32 dist) {
    ...
    if (runnerTagUpOrBatterTo1B(&outBase, &outFrames) == 0) {
        ...
        if (throwDist > 60.0f) {
            frames = 1.15f * throwDist / speed;
        } else if (throwDist > 30.0f) {
            frames = throwDist / speed;
        } else {
            frames = 0.8f * throwDist / speed;
        }
        if (throwDist > 80.0f) {
            frames *= 1.3f;
            frames += 0x1e;
        } else if (throwDist > 70.0f) {
            frames *= 1.15f;
            frames += 0x1e;
        }
        diff = frames + 0x14 - outFrames;
...
BOOL runnerTagUpOrBatterTo1B(int* outBase, int* outFrames) {
    ...
    }
    {
        ...
        if (runner->currentBase == throwLoc) {
            ...
            *outFrames = runner->framesToPreviousBase + runner->framesToPreviousBase + runner->framesToNextBase;
        }
        return 1;
    }
prevBaseFrames:
...
void setRunnerChasingAfter(int fielderIndex) {
    ...
    if (base >= 0) {
        ...
        if (forwards) {
            for (i = 0; i < 4; i++) {
                if (g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD && base == g_Runners[i].currentBase &&
                    g_Runners[i].baseStandingOn < 0 && fielder->distanceToBases[toBase] > g_Runners[i].distToNextBase) {
                    result = i;
                    break;
                }
            }
            ...
            for (i = 3; i >= 0; i--) {
                if (g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD && base == g_Runners[i].currentBase &&
                    g_Runners[i].baseStandingOn < 0 && fielder->distanceToBases[toBase] > g_Runners[i].distToCurrentBase) {
                    result = i;
                    break;
                }
            }
...
afterInput:
    ...
    if (g_FieldingLogic.locationThrownTo >= 0 && g_FieldingLogic.locationThrownTo <= 3 &&
        fielder->distanceToBases[g_FieldingLogic.locationThrownTo] <= 3.0f) {
...
void tagOutValues(int fielderIndex) {
    ...
    if (g_Strikes.outs < 3 && g_Ball.ballState == BALL_STATE_HELD && g_Ball.fielderWBallIndex >= 0 &&
        ...
        if (fielder->distanceToBases[0] <= 0.8f) {
            base = 0;
        } else if (fielder->distanceToBases[1] <= 0.8f) {
            base = 1;
        } else if (fielder->distanceToBases[2] <= 0.8f) {
            base = 2;
        } else if (fielder->distanceToBases[3] <= 0.8f) {
            base = 3;
        }
        if (base >= 0) {
            runner = &g_Runners[3];
            for (i = 3; i >= 0; i--) {
                if ((g_Pitcher.strikeOutOrWalk != 2 || runner->furthestBaseForcedToGoToOnWalk <= runner->currentBase) &&
                    ...
                    if (base == runner->baseStandingOn_Stored && runner->baseStandingOn < 0 &&
                        g_Ball.timeSinceBallPickedUp > 15 && runner->position.x < 20.0f) {
                    ...
                    if (base == runner->baseRunningTowards) {
                        ...
                        if (runner->actionCode != 0) {
                            ...
                            if (runner->tagUpInd == TAG_UP_TYPE_TAGGED && runner->distanceFromBall < 1.8f &&
                ...
                if (runner->runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
                    ...
                    if (calculateDiffInAngles(game_atan2(dx, dz), runDir) < 0x100 && runner->baseStandingOn < 0) {
                        if (runner->baseStandingOn < 0 || runner->tagUpInd == TAG_UP_TYPE_TAGGED) {
                            if (runner->distanceFromBall - fielder->currentVelocity < 1.8f) {
                                g_FieldingLogic.tagAnimationType = 2;
                                g_FieldingLogic.runnerBeingTargettedForOut = i;
                                g_FieldingLogic.tagResult_1out_2safe = 1;
                                g_FieldingLogic.framesRunnerIsOutBy = 1;
                        ...
                        if (dist < 5.0f) {
                            if (runner->baseStandingOn < 0) {
                                ...
                                if (runner->runningDirectionCode == 1 && runner->distToCurrentBase > 2.5f) {
                                    okToTag = 1;
                                }
                        ...
                        if (okToTag) {
                            result = 0;
                            if (runner->forceOutCd == FORCE_OUT_TYPE_FORCED_TO_ADVANCE &&
                                runner->baseStandingOn == runner->startingBase_baseAchieved && dist < 1.8f) {
                                result = 2;
                            } else {
                                if (g_FieldingLogic.somethingForTryingTagOutTargetBase < 0 ||
                                    g_FieldingLogic.somethingForTryingTagOutTargetBase > 3 ||
        ...
        if (g_FieldingLogic.tagAnimationType == 2 && 0.0f == fielder->currentVelocity) {
            g_FieldingLogic.tagAnimationType = 5;
        }
    }
    return;
tagFound:
    if (g_Runners[g_FieldingLogic.runnerBeingTargettedForOut].actionCode != 0) {
        if (g_Runners[g_FieldingLogic.runnerBeingTargettedForOut].runningDirectionCode == 1) {
            ...
            g_FieldingLogic.tagDirection.y = 0.0f;
...
void tagRelated(int fielderIndex) {
    ...
    if (baseOn >= 0) {
        g_Ball.baseBallAndFielderAreOn = baseOn;
        if (g_FieldingLogic.somethingForTryingTagOutTargetBase == baseOn) {
            g_FieldingLogic.somethingForTryingTagOutTargetBase = -1;
        }
    }
    if (g_FieldingLogic.framesRunnerIsOutBy <= 0) {
...
int fn_3_A6ABC(f32 x, f32 z) {
    ...
    speed = fielder->modifiedThrowingArm / 200.0f;
    dist = distanceXZ(x, z, fielder->pos.x, fielder->pos.z);
    if (0.0f == speed) {
        divisor = 1.0f;
    } else {
        divisor = speed;
    ...
    if (dist > 60.0f) {
        frames = 1.15f * dist / divisor;
    } else if (dist > 30.0f) {
        frames = dist / divisor;
    } else {
        frames = 0.8f * dist / divisor;
    }
    if (dist > 80.0f) {
        frames *= 1.3f;
        frames += 0x1e;
...
int estimatedThrowFramesBetweenTwoPoints(f32 ax, f32 az, f32 bx, f32 bz) {
    ...
    speed = throwSpeedArray[10] / 400.0f;
    dx = bx - ax;
    dz = bz - az;
    sqx = dx * dx;
    sqz = dz * dz;
...
int howManyFramesTheRunnerIsOutOfReach(int runnerIndex, BOOL forwards, int* throwStrat, int* runStrat) {
    ...
    if (coveringFielder == g_Ball.fielderWBallIndex || coveringFielder == -1 || ballDist < 3.0f) {
        *throwStrat = -1;
        *runStrat = base;
...
void fieldingAIThrowOrChase(int runnerIndex) {
    ...
    distFromBall = (int)runner->distanceFromBall;

    if (ballDistNext + ballDistCurr < 32.0f && runner->actionCode == 0) {
        ...
        if (runner->runningDirectionCode == 1) {
            if (runner->restrictedMovementCodes & 1) {
                ...
                if (g_Ball.baseBallAndFielderAreOn == baseNext &&
                    ((runnerDistNext < 7.0f && runner->runningDirectionCode == 1 &&
                ...
                } else if (runner->percentTowardsNextBase > 0.6f || !(frames < -15)) {
                    framesRunnerIsOutOfReach[runnerIndex] = frames;
                    throwStratToMakePlay[runnerIndex] = throwStrat;
                    runningStratToMakePlay[runnerIndex] = runStrat;
                } else {
            ...
            if (runner->restrictedMovementCodes & 2) {
                ...
                if (g_Ball.baseBallAndFielderAreOn == baseCurr && runnerDistCurr < 7.0f &&
                ...
                if (runner->percentTowardsNextBase < 0.2f || !(frames < -8)) {
                    framesRunnerIsOutOfReach[runnerIndex] = frames;
                    throwStratToMakePlay[runnerIndex] = throwStrat;
                    runningStratToMakePlay[runnerIndex] = runStrat;
                } else if (runner->percentTowardsNextBase < 0.3f && !(frames < -8) &&
        ...
        if (runner->percentTowardsNextBase < 0.25f) {
            if (ballDistNext < 10.0f) {
                runningStratToMakePlay[runnerIndex] = 9;
                throwStratToMakePlay[runnerIndex] = -1;
                framesRunnerIsOutOfReach[runnerIndex] = -15;
                return;
            }
            if (ballDistCurr < 10.0f) {
                if ((f32)distFromBall > 5.0f) {
        ...
        if (frames < -45) {
            if ((f32)distFromBall < 15.0f) {
                runningStratToMakePlay[runnerIndex] = 9;
                throwStratToMakePlay[runnerIndex] = -1;
                framesRunnerIsOutOfReach[runnerIndex] = -15;
            } else if (ballDistNext > 10.0f) {
                framesRunnerIsOutOfReach[runnerIndex] = frames;
                throwStratToMakePlay[runnerIndex] = throwStrat;
                runningStratToMakePlay[runnerIndex] = runStrat;
        ...
        } else if (runner->percentTowardsNextBase >= 0.45f) {
            framesRunnerIsOutOfReach[runnerIndex] = frames;
            throwStratToMakePlay[runnerIndex] = throwStrat;
            runningStratToMakePlay[runnerIndex] = runStrat;
        } else {
...
void runnerTimeToCoverBase(int runnerIndex) {
    ...
    dz = dz * dz;
    dist = dolsqrtf2(dx + dz);
    frames = throwFramesForDistance(dist, speed);
    totalFrames = fielder->throwWindupEstimate + frames;
    totalFrames = totalFrames + lbl_3_data_1C40;
    ballDist = g_Ball.ballDistanceFromBase[base];
    runFrames = fRunningTimeToDestinationPlus7(g_Ball.fielderWBallIndex, baseX, baseZ);
    if (coveringFielder == g_Ball.fielderWBallIndex || coveringFielder == -1 || ballDist < 3.0f ||
        ...
        runningStratToMakePlay[runnerIndex] = base;
        throwStratToMakePlay[runnerIndex] = -1;
        framesRunnerIsOutOfReach[runnerIndex] = runFrames - framesToBase;
    } else if (runFrames < coverFrames + 5) {
        runningStratToMakePlay[runnerIndex] = base;
        throwStratToMakePlay[runnerIndex] = -1;
        framesRunnerIsOutOfReach[runnerIndex] = runFrames - framesToBase;
    } else if (totalFrames + 5 < coverFrames) {
        throwStratToMakePlay[runnerIndex] = base + 10;
        runningStratToMakePlay[runnerIndex] = -1;
        framesRunnerIsOutOfReach[runnerIndex] = coverFrames - framesToBase;
    } else {
        throwStratToMakePlay[runnerIndex] = base;
        runningStratToMakePlay[runnerIndex] = -1;
        framesRunnerIsOutOfReach[runnerIndex] = totalFrames - framesToBase;
    }
}
...
void fn_3_A53DC(int runnerIdx) {
    ...
    if (runnerIdx == 3 && runner->baseStandingOn == 3 && g_RunningLogic._00 == 0x1000 &&
        ...
        frames = fielder->throwWindupEstimate + frames;
        frames = frames + lbl_3_data_1C40;
        throwStratToMakePlay[runnerIdx] = 0;
        runningStratToMakePlay[runnerIdx] = -1;
        framesRunnerIsOutOfReach[runnerIdx] = frames - runner->framesToNextBase;
    }
}
...
void throwTimeToBase(int baseCounter) {
    ...
    dist = dolsqrtf2(dx + dz);
    frames = throwFramesForDistance(dist, speed);
    totalFrames = frames + 0x14;
    totalFrames = totalFrames + fielder->throwWindupEstimate;
    totalFrames = totalFrames + lbl_3_data_1C40;
    ballDist = g_Ball.ballDistanceFromBase[base];
    runFrames = fRunningTimeToDestinationPlus7(g_Ball.fielderWBallIndex, baseX, baseZ);
    ...
    if (coveringFielder == g_Ball.fielderWBallIndex || coveringFielder == -1 || ballDist < 4.0f ||
        ((coverFrames = g_Fielders[coveringFielder].numFramesToGetToAutoLocation) < 0 &&
         g_FieldingLogic.baseCoveredInd[base] == 0) ||
        g_Ball.framesSinceHit < 0x46) {
        runningStratToMakePlay[baseCounter] = base;
        throwStratToMakePlay[baseCounter] = -1;
        framesRunnerIsOutOfReach[baseCounter] = runFrames - framesToBase;
    } else if (runFrames < coverFrames + 5) {
        runningStratToMakePlay[baseCounter] = base;
        throwStratToMakePlay[baseCounter] = -1;
        framesRunnerIsOutOfReach[baseCounter] = runFrames - framesToBase;
        ...
        throwStratToMakePlay[baseCounter] = base + 10;
        runningStratToMakePlay[baseCounter] = -1;
        framesRunnerIsOutOfReach[baseCounter] = coverFrames - framesToBase;
    } else {
        throwStratToMakePlay[baseCounter] = base;
        runningStratToMakePlay[baseCounter] = -1;
        framesRunnerIsOutOfReach[baseCounter] = totalFrames - framesToBase;
    }
}
    ...
    if (strat >= 0) {
        ...
        if (g_FieldingLogic.somethingForTryingTagOutTargetBase == 9) {
            if (runner->runningDirectionCode == 2) {
                ...
                setFielderAutoCoords(g_Ball.fielderWBallIndex, 2.0f * vx + runner->position.x,
                                     2.0f * vz + runner->position.z);
            }
        }
    } else {
        g_FieldingLogic.somethingForTryingTagOutTargetBase = -1;
...
void fn_3_A41E8(int base) {
    ...
    if (base == 5) {
        ...
        if (g_FieldingLogic.fielderAssignedLocationIndex[4] == -1) {
            ...
            cover = 0;
        } else if (fielder->distanceToMound < 10.0f) {
            cover = 0;
        } else {
            cover = 1;
        }
    ...
    if (assigned == g_Ball.fielderWBallIndex || assigned == -1 || ballDist < 3.0f) {
...
void fielderAIChaseRunner(void) {
    ...
    if (g_FieldingLogic.somethingForTryingTagOutTargetBase >= 0 && g_FieldingLogic.somethingForTryingTagOutTargetBase <= 3) {
        ...
        if (assigned != fielderIdx) {
            if (fielder->distanceToBases[g_FieldingLogic.somethingForTryingTagOutTargetBase] < 3.0f) {
                s16 prev = assigned;
...
BOOL fielderAIOutfieldPlayAttemptInd(void) {
    ...
    if ((hexBaserunnerTracker & 0x1000) != 0) {
        if ((hexBaserunnerTracker & 0xFFF) == 0 && g_Ball.AtBat_ContactResult == BALL_RESULT_TYPE_CAUGHT &&
            ...
            f32 maxDist = 0.35f * (f32)fielder->throwingArm + 69.0f;

            if (g_Ball.ballDistanceFromBase[0] >= 62.0f && g_Ball.ballDistanceFromBase[0] <= maxDist &&
        ...
        if ((hexMovementsInEachBaseline1for2back4stop & 0x1000) != 0) {
            if (urgencyLevel == 3) {
                if (g_Ball.ballDistanceFromBase[0] < 75.0f &&
                ...
                return 0;
            } else if (g_Ball.ballDistanceFromBase[0] < 68.0f) {
                if ((hexBaserunnerTracker & 0x110) != 0) {
                    return 0;
                }
                if (fielderDesperationLevel[originalBaseOfRunnerInThisBaseline[3]] <= 4 &&
                    fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[3]) != 0) {
    ...
    if ((hexBaserunnerTracker & 0x100) != 0 && g_Ball.ballDistanceFromBase[3] < 65.0f &&
    ...
    if ((hexBaserunnerTracker & 0x10) != 0 && g_Ball.ballDistanceFromBase[2] < 50.0f &&
...
BOOL fn_3_A32B8(void) {
    ...
    if ((hexMovementsInEachBaseline1for2back4stop & 0x1000) != 0 &&
        g_Runners[originalBaseOfRunnerInThisBaseline[3]].percentTowardsNextBase >= 0.2f &&
        fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[3]) != 0) {
        return 1;
    }
    return 0;
    ...
    if (g_Ball.ballDistanceFromBase[0] <= g_Ball.ballDistanceFromBase[2]) {
        ...
        if ((hexMovementsInEachBaseline1for2back4stop & 0x6000) != 0 &&
            ...
            if ((g_FieldingLogic.fielderAssignedLocationIndex[3] < 0 ||
                 g_Fielders[g_FieldingLogic.fielderAssignedLocationIndex[3]].distanceToBases[3] > 4.0f) &&
...
BOOL fn_3_A2C9C(void) {
    if ((hexBaserunnerTracker & 0x100) != 0 && g_Ball.ballDistanceFromBase[3] < 5.0f &&
        fielderDesperationLevel[originalBaseOfRunnerInThisBaseline[2]] <= 2 &&
        fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[2]) != 0) {
        return 1;
    }
    ...
    if (fielderDesperationLevel[originalBaseOfRunnerInThisBaseline[1]] <= 4 && g_Strikes.outs == g_Strikes.storedOuts) {
        if (g_Ball.ballDistanceFromBase[1] < 2.5f &&
...
BOOL fn_3_A2B6C(void) {
    if (g_Runners[originalBaseOfRunnerInThisBaseline[3]].percentTowardsNextBase >= 0.25f) {
        ...
        if ((hexMovementsInEachBaseline1for2back4stop & 0x6000) != 0 &&
            ...
            !(g_Fielders[g_FieldingLogic.fielderAssignedLocationIndex[3]].distanceToBases[3] > 4.0f) &&
            !(g_Runners[originalBaseOfRunnerInThisBaseline[3]].percentTowardsNextBase < 0.5f &&
...
BOOL genericPlayOnRunnerOffBase(void) {
    ...
    do {
        if (originalBaseOfRunnerInThisBaseline[3] >= 0 &&
            ...
            if ((g_Runners[originalBaseOfRunnerInThisBaseline[3]].percentTowardsNextBase > 0.2f ||
            ...
            if ((hexMovementsInEachBaseline1for2back4stop & 0x1000) != 0 &&
                g_Runners[originalBaseOfRunnerInThisBaseline[3]].percentTowardsNextBase > 0.15f &&
                fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[3]) != 0) {
                who = 3;
                break;
            }
        ...
        if (originalBaseOfRunnerInThisBaseline[2] >= 0 &&
            ...
            if ((g_Runners[originalBaseOfRunnerInThisBaseline[2]].percentTowardsNextBase > 0.2f ||
            ...
            if ((hexMovementsInEachBaseline1for2back4stop & 0x100) != 0 &&
                g_Runners[originalBaseOfRunnerInThisBaseline[2]].percentTowardsNextBase >= 0.15f &&
                fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[2]) != 0) {
                who = 2;
                break;
            }
        ...
        if (originalBaseOfRunnerInThisBaseline[1] >= 0 &&
            fielderDesperationLevel[originalBaseOfRunnerInThisBaseline[1]] <= 5) {
            if ((g_Runners[originalBaseOfRunnerInThisBaseline[1]].percentTowardsNextBase > 0.2f ||
            ...
            if ((hexMovementsInEachBaseline1for2back4stop & 0x10) != 0 &&
                g_Runners[originalBaseOfRunnerInThisBaseline[1]].percentTowardsNextBase > 0.15f &&
                fielderAIMakePlay(originalBaseOfRunnerInThisBaseline[1]) != 0) {
                who = 1;
                break;
            }
    ...
    if (g_Ball.ballDistanceFromBase[0] < 3.0f && g_FieldingLogic.locationThrownTo == 2 &&
        originalBaseOfRunnerInThisBaseline[3] >= 0 && who == 1) {
        if (g_Runners[originalBaseOfRunnerInThisBaseline[who]].fractionalBasesRan <= 1.2f) {
            BOOL cutoffReady;
...
BOOL canThrowOutRunnerWhoNeedsToTagUp(void) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (g_Runners[base].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
            g_Runners[base].tagUpInd == TAG_UP_TYPE_TAGGED && fielderDesperationLevel[base] <= 4 &&
            g_Ball.ballDistanceFromBase[base] < 45.0f && fielderAIMakePlay(base) != 0) {
            return 1;
        }
        p++;
    }
...
BOOL fn_3_A1DA0(void) {
    ...
    if (fielder->distanceFromHomePlate > 80.0f - 0.1f * (100 - fielder->throwingArm)) {
        return 0;
    }
    if (g_Runners[3].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
        ...
        (g_Runners[3].runningDirectionCode == 2 || g_Runners[3].runningDirectionCode == 1)) {
               ...
               g_Runners[2].fractionalBasesRan > 2.85f &&
               g_Runners[2].fractionalBasesRan <= 3.15f && g_Runners[3].runningDirectionCode == 1 &&
    ...
    if (g_Runners[1].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD) {
        if (g_Runners[1].tagUpInd != TAG_UP_TYPE_NONE) {
            ...
            if (g_Runners[1].fractionalBasesRan <= 0.7f) {
                return 0;
            }
        }
    }
...
int fn_3_A1D04(void) {
    ...
    for (i = 3; i >= 0; i--) {
        if (g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD) {
            f32 pos = g_Runners[i].fractionalBasesRan;
            f32 diff = pos - best;
            if (diff < 0.0f) {
                diff = -diff;
            }
...
void fielderAIDecideWhatRunnerToTarget(int fielderIndex) {
    ...
    if (g_FieldingLogic.throwWaitingInd != 0) {
        ...
        f32 rangeF = (f32)(lbl_3_data_4710[1] - lbl_3_data_4710[0]);
        int limit;

        rangeF *= g_AiLogic.aIDifficultyMultiplierArray[g_GameLogic.awayTeamBattingInd_battingTeam];
        limit = lbl_3_data_4710[0] + (int)rangeF;
                    ...
                    if (fielderDesperationLevel[0] >= 4) {
                        ...
                        do {
                            if (runner->runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
                                ...
                                fielder->distanceToBases[runner->nextBase] < 3.0f &&
                                fielderDesperationLevel[*base] <= 3 && runner->distanceFromBall < 5.0f &&
                                fielderAIMakePlay(*base) != 0) {
...
checkOverrideReasons:
    ...
    if (g_FieldingLogic.locationThrownTo == 6) {
        if (g_FieldingLogic.cutoffFielderIndex < 0) {
            ...
            if (5.0f + g_Fielders[g_FieldingLogic.cutoffFielderIndex].distanceToMound >
                fielder->distanceToMound) {
                g_FieldingLogic.locationThrownTo = -1;
            }
            if (fielder->distanceFromEachFielder[g_FieldingLogic.cutoffFielderIndex] < 5.0f) {
                g_FieldingLogic.locationThrownTo = -1;
            }
        }
    }
