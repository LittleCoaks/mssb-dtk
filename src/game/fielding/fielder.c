...
extern const f32 lbl_3_rodata_C1C;
extern const f32 lbl_3_rodata_C20;
extern const f32 lbl_3_rodata_C6C;
...
extern const f32 lbl_3_rodata_C64;
extern const f32 lbl_3_rodata_C2C;
extern const f32 lbl_3_rodata_C30;
extern const f32 lbl_3_rodata_C34;
extern const f32 lbl_3_rodata_C68;
extern const f32 lbl_3_rodata_BA8;
extern const f32 lbl_3_rodata_BAC;
...
extern const f32 lbl_3_rodata_CC0;
...
int wallJumpInitialization(int fielderIndex) {
    ...
    if (g_Ball.physicsSubstruct.futureCoordsAndDist[idx].pos.y <
        fielderActionConstants[8] + fielder->hitbox[3]) {
        return 0;
    }
...
farWall:
    ...
    magnitude = fielderSqrt(SQ(collision.normal.x) + SQ(collision.normal.z));
    fielder->wallJumpApexX =
        fielder->wallActionLocationX + (collision.normal.x / magnitude) * fielderActionConstants[6];
    fielder->wallJumpApexY = fielder->wallActionLocationY + fielderActionConstants[8];
    fielder->wallJumpApexZ =
        fielder->wallactionLocationZ + (collision.normal.z / magnitude) * fielderActionConstants[6];
...
nearWall:
    ...
    inVec.src.y = -fielderActionConstants[5];
    inVec.src.z = fielder->pos.z;
    inVec.dst.x = fielder->pos.x + (dx / magnitude) * fielderActionConstants[4];
    inVec.dst.z = fielder->pos.z + (dz / magnitude) * fielderActionConstants[4];
    ...
    fielder->wallJumpApexY = ballY - lbl_3_rodata_B68;
...
int ballThrownToEmptyBaseCatchAttempt(int fielderIndex) {
    ...
    travelDist = lbl_3_rodata_B84 * fielder->joggingSpeed * (f32)g_Ball.framesUntilThrowReachesDest;

    if (remainingDist - travelDist > lbl_3_rodata_B78) {
        return 0;
    }
    ...
    if (remainingDist < travelDist) {
        ...
        if (remainingDist / (f32)g_Ball.framesUntilThrowReachesDest < lbl_3_rodata_B6C) {
        ...
        fielder->actionStartingCoordinate.x =
            fielder->pos.x +
            lbl_3_rodata_B88 * fielder->joggingSpeed * (dx / dist) * (f32)g_Ball.framesUntilThrowReachesDest;
        fielder->actionStartingCoordinate.z =
            fielder->pos.z +
            lbl_3_rodata_B88 * fielder->joggingSpeed * (dz / dist) * (f32)g_Ball.framesUntilThrowReachesDest;
    }
...
int catchThrownBallFun(int fielderIndex) {
    InMemFielder* fielder = &g_Fielders[fielderIndex];
    f32 heightThreshold = lbl_3_rodata_B84 * fielder->hitbox[3];
...
foundCatch:
...

    {
...

        {
            ...
            zeroVec.x = lbl_3_rodata_B20;
            zeroVec.y = lbl_3_rodata_B20;
            zeroVec.z = lbl_3_rodata_B20;
            targetPos.x = *entryX;
            targetPos.y = lbl_3_rodata_B20;
            targetPos.z = *entryZ;
...
void checkForCatchBallAction(int fielderIndex) {
    ...
    if (g_d_GameSettings.GameModeSelected == GAME_TYPE_PRACTICE && fielderIndex >= 1) {
        if (g_Practice.practiceType_2 == PRACTICE_TYPE_PITCHING || g_Practice.practiceType_2 == PRACTICE_TYPE_BATTING) {
            return;
        }
...
void moveFieldersFromDugoutToStartingPositions(void) {
    ...
    for (i = 0; i < 9; i++) {
        ...
        if (dist < lbl_3_rodata_BB8 ||
            fielder->unused_rngValueRelatedToDugoutLocation > g_GameLogic.FrameCountOfCurrentPitch) {
            fielder->velocityX = lbl_3_rodata_B20;
            fielder->velocityZ = lbl_3_rodata_B20;
            fielder->currentVelocity = lbl_3_rodata_B20;
        } else {
            fielder->velocityX = lbl_3_rodata_B74 * (dx / dist) - lbl_3_rodata_B48;
            fielder->pos.x += fielder->velocityX;
            fielder->velocityZ = lbl_3_rodata_B74 * (dz / dist) - lbl_3_rodata_B48;
            fielder->pos.z += fielder->velocityZ;
...
void fielding_atBat_SetSomeAutomovements_callCollisionFn(void) {
    ...
    for (fielder = g_Fielders, i = 0; i < 9; fielder++, i++) {
        ...
        if (!minigamesEnabled || matchIdx < 4) {
            if (fielder->currentVelocity > lbl_3_rodata_B20) {
                if (pauseControl[0x1d5] == 0) {
                ...
                fielder->desiredMovementDirection = lbl_3_rodata_BC0;
            }
            ...
            if (fielder->velocityX != lbl_3_rodata_B20 || fielder->velocityZ != lbl_3_rodata_B20) {
                ...
                fielder->desiredMovementDirection2 = lbl_3_rodata_B20;
...
void humanTeamFieldingFirstFrameAfterHit(void) {
    ...
    for (i = 0, shiftAmt = 0; i < 9; i++, shiftAmt += 3) {
        ...
        } else if (g_Fielders[i].pos.x == lbl_3_rodata_B20) {
            g_Fielders[i].angleOfFieldersStartingPosition = SANG_ANG_90;
        } else {
            angle = (int)(lbl_3_rodata_BC4 * ATAN2F(g_Fielders[i].pos.z, g_Fielders[i].pos.x) / const_pi);
            if (angle < 0) {
                angle += SANG_ANG_180;
            }
    ...
    if (g_Ball.lineDriveThroughPitcherInd) {
        ...
        setInitialFielderMovements_foulBall();
               ...
               g_Ball.maxYOfHit > f8_0) {
        if (g_Ball.maxYOfHit < lbl_3_rodata_B64) {
            setInitialFielderMovements_generalCase(1);
            chooseSecondaryFielderFromClosestOutfielder();
            index2 = g_FieldingLogic.secondaryFielder;
            f2 = &g_Fielders[index2];
...
void decideHowToTrackFoulBall(int fielderIndex) {
    extern const f32 lbl_3_rodata_BE0;
    extern const f32 lbl_3_rodata_B9C;
    InMemFielder* fielder = &g_Fielders[fielderIndex];

    if (g_Ball.fielderWBallIndex >= 0 && g_Ball.fielderWBallIndex != fielderIndex) {
        fielder->distanceFromAutoLocation = lbl_3_rodata_B20;
        fielder->IntendedLocation = fielder->pos;
        fielder->velocityX = lbl_3_rodata_B20;
        fielder->velocityZ = lbl_3_rodata_B20;
    } else {
        fielder->fielderVeloAdjustmentCode = 0;
        ...
        if (g_Ball.fielderAboutToGetBall_hasBall >= 0 && g_Ball.AtBat_ContactResult == BALL_RESULT_TYPE_IN_AIR &&
            ...
            if (g_Ball.ballVelocity <= lbl_3_rodata_B20 && dist <= lbl_3_rodata_B9C) {
            ...
            } else if (g_Ball.physicsSubstruct.hitLandingSpotDistFromHome >= lbl_3_rodata_B78) {
                f32 d = g_Ball.physicsSubstruct.hitLandingSpotDistFromHome;
                f32 targetX = g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.x +
                              lbl_3_rodata_B70 * (g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.x / d);
                f32 targetZ = g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.z +
                              lbl_3_rodata_B70 * (g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.z / d);
                fielderTrackingBall_applyIntendedLocation(fielder, targetX, targetZ, 0);
            } else if (fielder->distanceFromHomePlate >= lbl_3_rodata_B70) {
                f32 targetX = lbl_3_rodata_B70 * (fielder->pos.x / dist);
                f32 targetZ = lbl_3_rodata_B70 * (fielder->pos.z / dist);
                fielderTrackingBall_applyIntendedLocation(fielder, targetX, targetZ, 0);
            } else {
                fielderTrackingBall_applyIntendedLocation(fielder, fielder->pos.x, fielder->pos.z, 0);
            }

            if (fielder->distanceFromAutoLocation >= lbl_3_rodata_BA0) {
            ...
            BOOL atLandingSpot = (g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.x == fielder->pos.x &&
                                   g_Ball.physicsSubstruct.ballLandingSpotOrHeldSpot.z == fielder->pos.z);
            int n;
            ...
            if (g_Ball.framesUntilBallHitsGround - n > 10) {
                ...
                for (i = 10; i <= 60; i += 10) {
                    ...
                    if (i == 60) {
                        ...
                        BOOL atSpot = (c->pos.x == fielder->pos.x && c->pos.z == fielder->pos.z);
                        int n2;
                        ...
                        if (atSpot) {
                            ...
                            n2 = (fielder->maxAccLength_ConstF >> 1) + (int)(fielderSqrt(dx * dx + dz * dz) / speed);
                        }
            ...
            if (fielderIndex <= 5) {
                if (fielder->distanceFromLandingSpot >= lbl_3_rodata_BA0) {
                    fielder->fielderVeloAdjustmentCode = 8;
                }
            } else {
                BOOL handled = 0;
            ...
            f32 targetX;
            f32 targetZ;
            f32 factor = lbl_3_rodata_B60 +
                         ((fielder->groundDistanceFromBall - lbl_3_rodata_B70) / lbl_3_rodata_BDC) +
                         (g_Ball.ballVelocity - lbl_3_rodata_BE0);
            ...
            if (fielderIndex >= 6 && fielder->distanceFromHomePlate < const50_0) {
            ...
            if (fielder->distanceFromBall < lbl_3_rodata_B9C && fielder->groundDistanceFromBall < lbl_3_rodata_B64) {
    ...
    if (fielder->currentVelocity < lbl_3_rodata_B48 && fielder->fielderVeloAdjustmentCode == 8) {
        fielder->distanceFromAutoLocation = lbl_3_rodata_B20;
        fielder->currentVelocity = lbl_3_rodata_B20;
        fielder->IntendedLocation.x = fielder->pos.x;
        fielder->IntendedLocation.z = fielder->pos.z;
...
void autoMovement21_23_humanControlInitialSelectedFielders(int fielderIndex) {
    ...
    if (result != 1) {
        if (fielder->fielderTrackingBallState == 1) {
            ...
            fielder->movementAngle =
                (g_FieldingLogic.playOverInd != 0) ? -1 : fielderControlStick_continuousAngleHistory[0];
    ...
    loc = fielder->locationResponsibleForCovering;
    if (loc >= 0 && loc <= 3 && fielder->distanceToBases[loc] > lbl_3_rodata_B78) {
        if (loc >= 0 && loc <= 3) {
...
void autoMovement15_selectedFielderOnLooseBall(int fielderIndex) {
    ...
    if (!(fielder->distanceToBases[loc] > lbl_3_rodata_B78)) {
        return;
    }
...
void checkIfPlayerNeedsToMoveToCatchBall(int fielderIndex) {
    ...
    if (fielder->animationRelatedInd == 0) {
        if (g_Ball.framesSinceThrowStarted == 5) {
            if (lbl_3_rodata_B20 != g_Ball.ballVelocity) {
                ...
                if (dist > fielder->hitbox[0]) {
                    ...
                    if (lbl_3_rodata_B20 == dx2 && lbl_3_rodata_B20 == dz2) {
                        fielder->currentVelocity = lbl_3_rodata_B20;
                        fielder->distanceFromAutoLocation = lbl_3_rodata_B20;
                    } else {
...
void autoMovement14_infieldSupport(int fielderIndex) {
    ...
    if (location == 5) {
        if (g_FieldingLogic.locationThrownTo == 5) {
            ...
            offsetX = lbl_3_rodata_B68;
            if (fielder->pos.x < lbl_3_rodata_B20) {
                offsetX = lbl_3_rodata_C1C;
            }
            offsetZ = lbl_3_rodata_B68;
            if (fielder->pos.z < base_MoundCoordinates[4].z) {
                offsetZ = lbl_3_rodata_C1C;
            }
            ...
            if (fielder->distanceFromAutoLocation < lbl_3_rodata_B78) {
...
void fn_3_3FCF0(BOOL useComputedAngle) {
    ...
    if (g_Fielders[0].cantCatchFlyBallInd == 0 &&
        g_Ball.physicsSubstruct.hitLandingSpotDistFromHome < lbl_3_rodata_B90 && g_Ball.maxYOfHit < f8_0) {
        fn_3_3F760();
        return;
    }
...
void fn_3_402A8(void) {
    ...
    if ((mask & 0x9249) == 0 || g_Ball.landingSpotZoneAwayFromHome > 1 || !(g_Ball.maxYOfHit >= f8_0)) {
        if (g_Fielders[0].nonCatchFlyBallStratInd != 0 || g_Fielders[1].nonCatchFlyBallStratInd != 0) {
            goto farTail;
        }
        if (!(g_Ball.physicsSubstruct.hitLandingSpotDistFromHome < lbl_3_rodata_C28)) {
            goto farTail;
        }
    }
    ...
    if (g_Fielders[1].nonCatchFlyBallStratInd == 0 &&
        (g_Ball.physicsSubstruct.hitLandingSpotDistFromHome < lbl_3_rodata_BE4 ||
    ...
    if ((dist < lbl_3_rodata_C2C && angle > 0x300 && angle < 0x500) ||
        (dist < lbl_3_rodata_C30 && angle > 0x300 && angle < 0x500 && (mask & 1) != 0) ||
        (dist < lbl_3_rodata_C34 && angle > 0x200 && angle < 0x600)) {
        setFielderAutoMovement(0, 3);
    } else if (angle < 0x2c0) {
        setFielderAutoMovement(2, 3);
...
farTail: {
    ...
    if ((mask & 0x1240000) != 0) {
        ...
        if (g_Ball.Hit_HorizontalAngle < 0x3c0) {
            ...
            dist3 = fielderSqrt(dx3 * dx3 + dz3 * dz3);

            dx5 = g_Fielders[5].pos.x - ball->landingSpotLocation.x;
            dz5 = g_Fielders[5].pos.z - ball->landingSpotLocation.z;
            dist5 = fielderSqrt(dx5 * dx5 + dz5 * dz5);

            if (g_Ball.landingSpotZoneAwayFromHome < 3 || !(g_Ball.maxYOfHit >= f8_0)) {
                if (dist3 < dist5) {
                    setFielderAutoMovement(3, 3);
                } else {
                    setFielderAutoMovement(5, 3);
...
void aITeamFieldingFirstFrameAfterHit(void) {
    ...
    if (g_Scores._pad_50[0x5A] <= g_Scores.Inning && g_Scores.halfInning != 0 &&
        ...
        g_Ball.physicsSubstruct.hitLandingSpotDistFromHome > lbl_3_rodata_C00) {
...
void setIntendedLocationAheadOfBallPath(int fielderIndex, f32* outX, f32* outZ) {
...

    {
        ...
        if (fielderIndex >= 6) {
            ...
            f32 remaining = lbl_3_rodata_C00 - mag;
...
void autoMovement13_GoTowardsBallAsBackUp(int fielderIndex) {
    ...
    default:
        if (fielder->AI_Ind == 0) {
            ...
            outX = fielder->pos.x;
            outZ = fielder->pos.z;
        } else if (fielder->distanceFromBall < lbl_3_rodata_B64) {
            outX = fielder->pos.x;
            outZ = fielder->pos.z;
        } else if (g_Ball.ballState == BALL_STATE_LOOSE) {
            outX = g_Ball.AtBat_Contact_BallPos.x + lbl_3_rodata_BA0 * g_Ball.ballVelocityPercent.x;
            outZ = g_Ball.AtBat_Contact_BallPos.z + lbl_3_rodata_BA0 * g_Ball.ballVelocityPercent.z;
        } else if (lbl_3_rodata_B20 == fielder->currentVelocity &&
                   fielder->distanceFromEachFielder[otherFielderIndex] < lbl_3_rodata_B70) {
            outX = fielder->pos.x;
            outZ = fielder->pos.z;
        } else {
            outX = g_Fielders[otherFielderIndex].IntendedLocation.x + lbl_3_rodata_BA0 * g_Ball.ballVelocityPercent.x;
            outZ = g_Fielders[otherFielderIndex].IntendedLocation.z + lbl_3_rodata_BA0 * g_Ball.ballVelocityPercent.z;
        }
        ...
        if (fielderIndex >= 6) {
            ...
            if (otherFielderIndex >= 6 && fielder->distanceFromAutoLocation < lbl_3_rodata_B9C) {
                fielder->fielderVeloAdjustmentCode = 5;
            }

            if (fielder->distanceFromAutoLocation < lbl_3_rodata_B70) {
                fielder->fielderVeloAdjustmentCode = 7;
            } else if (fielder->distanceFromAutoLocation < lbl_3_rodata_B9C) {
                fielder->fielderVeloAdjustmentCode = 6;
            }
            ...
            if (otherFielderIndex <= 5 && fielder->distanceFromAutoLocation < lbl_3_rodata_BA0) {
                fielder->fielderVeloAdjustmentCode = 7;
            }
        }

        for (i = 0; i < 9; i++) {
            if (i != fielderIndex && lbl_3_rodata_B78 > fielder->distanceFromEachFielder[i]) {
                fielder->fielderVeloAdjustmentCode = 9;
                break;
            }
        }
    ...
    if (g_Ball.AtBat_ContactResult != BALL_RESULT_TYPE_IN_AIR) {
        if (fielder->distanceFromEachFielder[otherFielderIndex] > lbl_3_rodata_B64 + fielder->distanceFromBall) {
            f32 dist;
            f32 dxSq;
            f32 dzSq;
...
void autoMovement6_outfielderNoCatch_Phase2(int fielderIndex) {
    ...
    default:
        if (g_FieldingLogic.locationThrownTo >= 1 && g_FieldingLogic.locationThrownTo <= 3 &&
            ...
            targetX = moundX + lbl_3_rodata_B9C * outX;
            targetZ = moundZ + lbl_3_rodata_B9C * outZ;
            ...
            if (angle2 <= 0x800 &&
                ...
                if (targetZ < base_MoundCoordinates[1].z) {
                    targetX = (targetX < lbl_3_rodata_B20) ? lbl_3_rodata_C64 : lbl_3_rodata_C68;
                    targetZ = lbl_3_rodata_BE8;
                }
...
void autoMovement1_coverBase(int fielderIndex) {
...

    {
        ...
        if (fielderIndex != 0 && g_Ball.framesSinceHit < lockout - 15) {
            return;
        }

        if (fielderIndex == 0 && g_Ball.framesSinceHit < lockout) {
            return;
        }
    }
    ...
    if (fielder->isResponsibleForCoveringALocation == 1) {
        if (fielder->needToMoveToCatchThrownBall != 0) {
            ...
            if (g_FieldingLogic.baseCoveredInd[fielder->locationResponsibleForCovering] == 1) {
                s16 loc = fielder->locationResponsibleForCovering;
                ...
                if (fielder->distanceToBases[loc] >
                    ...
                    fielder->velocityX = (lbl_3_rodata_C20 * diffX) / fielder->distanceToBases[loc];
                    fielder->velocityZ = (lbl_3_rodata_C20 * diffZ) / fielder->distanceToBases[loc];
                    fielder->currentVelocity = lbl_3_rodata_C20;
                    ...
                    if (dist >= lbl_3_rodata_B24) {
                        fielder->velocityX = diffX / lbl_3_rodata_B64;
                        fielder->velocityZ = diffZ / lbl_3_rodata_B64;
                        fielder->pos.x += fielder->velocityX;
                        fielder->pos.z += fielder->velocityZ;
                    } else {
        ...
        } else if (fielder->pitcherHeadingToCover1stOr3rd == 1) {
            f32 x;
            f32 z;
            f32 diffX;
            f32 diffZ;
            ...
            if (pitcher->locationResponsibleForCovering == 1) {
                ...
                if (d > lbl_3_rodata_B70) {
                    d = lbl_3_rodata_B70;
                }

                x = base_MoundCoordinates[1].x;
                z = base_MoundCoordinates[1].z - d / lbl_3_rodata_B78;
                ...
                z = base_MoundCoordinates[3].z - d / lbl_3_rodata_B78;
            } else {
            ...
            if (lbl_3_rodata_B20 == diffX && lbl_3_rodata_B20 == diffZ) {
                pitcher->currentVelocity = lbl_3_rodata_B20;
                pitcher->distanceFromAutoLocation = lbl_3_rodata_B20;
            } else {
...
void checkForAndHandleLooseBalls(void) {
    ...
    if (!((g_Ball.framesSinceThrowStarted > 5 && fielder->distanceFromBall > fielder->distanceFromBallLastFrame) ||
          (g_Ball.framesUntilThrowReachesDest < 1 && fielder->distanceFromBall > lbl_3_rodata_B78) ||
          (g_Ball.ballVelocity < lbl_3_rodata_C6C && g_Ball.fielderWBallIndex < 0 &&
           g_Ball.framesSinceThrowStarted > 60) ||
          (dist > lbl_3_rodata_B68 + g_Ball.throwDistance))) {
    ...
    if (g_Ball.framesSinceThrowStarted > 5 &&
        (lbl_3_rodata_B78 + throwDist < distBallToThrowStart ||
...
void setCatcherCatchStrategy_distToLandingSpot(int fielderIndex) {
    ...
    for (frame = startFrame; frame < 360;) {
        ...
        if (framesNeeded > lastFramesNeeded && found == 0) {
            ...
            if (originDist > lbl_3_rodata_BA0) {
                resultCode = 3;
                break;
            }
        }
...
void setFielderCatchStrategy_calcFramesToGetToDropSpot(int fielderIndex) {
    ...
    s16 delayFrames;
    f32 bestHeight = lbl_3_rodata_B94;
    f32 threshold = lbl_3_rodata_B98;
    f32 dx;
    f32 dz;
    f32 dist;
    f32 speed;
    ...
    if (g_d_GameSettings.minigamesEnabled) {
        ...
        switch (fielder->AILevel3Weak0Powerful) {
        case 0:
            step = 2;
            break;
        case 1:
        ...
        if (fielderIndex >= 6) {
            threshold = fielder->distanceFromHomePlate - lbl_3_rodata_B24;
        }
    }
    ...
    fielderX = fielder->pos.x;
    fielderZ = fielder->pos.z;
    ...
    for (frame = startFrame; frame < 360;) {
        ...
        if (futureCoord->pos.x == fielderX && futureCoord->pos.z == fielderZ) {
            framesNeeded = 1;
        } else {
            f32 dxSq;
            f32 dzSq;
            dx = futureCoord->pos.x - fielderX;
            dz = futureCoord->pos.z - fielderZ;
            dxSq = dx * dx;
            dzSq = dz * dz;
            dist = fielderSqrt(dxSq + dzSq);
        ...
        if (framesNeeded < frame - delayFrames) {
            ...
            if (framesNeeded < frame + 30) {
                if (fielderIndex < 2 || fielderIndex > 5) {
                    closeEnough = 1;
                } else if (futureCoord->dist <= lbl_3_rodata_B64 + fielder->distanceFromHomePlate) {
                    closeEnough = 1;
                } else if (framesNeeded < frame + 10) {
                    closeEnough = 1;
                }
            }
        ...
        if (framesNeeded > lastFramesNeeded && found == 0 &&
            ...
            for (i = 0, offset = 5; i < 14; i++, offset += 5) {
                ...
                CoordAndDist* candidate = &g_Ball.physicsSubstruct.futureCoordsAndDist[candidateFrame];
                int candidateFramesNeeded;

                if (candidate->pos.x == fielderX && candidate->pos.z == fielderZ) {
                    candidateFramesNeeded = 1;
                } else {
                    f32 cdx = candidate->pos.x - fielderX;
                    f32 cdz = candidate->pos.z - fielderZ;
                    f32 cdxSq = cdx * cdx;
                    f32 cdzSq = cdz * cdz;
                    f32 cdist = fielderSqrt(cdxSq + cdzSq);
                    f32 cspeed = fielder->joggingSpeed;
                    int ctravelFrames;
                    if (cspeed == lbl_3_rodata_B20) {
                        cspeed = lbl_3_rodata_B60;
                    }
...

            {
                ...
                for (j = 0; j < 5; j++, offset2 += 5) {
                    ...
                    if (candidate->pos.x == fielderX && candidate->pos.z == fielderZ) {
                        ...
                        f32 cdx = candidate->pos.x - fielderX;
                        f32 cdz = candidate->pos.z - fielderZ;
                        f32 cdist = fielderSqrt(cdx * cdx + cdz * cdz);
                        f32 cspeed = fielder->joggingSpeed;
                        int ctravelFrames;
                        if (cspeed == lbl_3_rodata_B20) {
                            cspeed = lbl_3_rodata_B60;
...

    {
        ...
        if (landingDist < lbl_3_rodata_B8C && resultCode == 1) {
            fielder->nonCatchFlyBallStratInd = 0;
        } else if (landingDist < lbl_3_rodata_B64 && resultCode == 1 && fielderIndex >= 6) {
            fielder->nonCatchFlyBallStratInd = 0;
        } else {
            fielder->nonCatchFlyBallStratInd = 1;
        }
    ...
    if (g_Ball.howFoulTheBallWillBe != 0 && g_Ball.maxYOfHit >= lbl_3_rodata_B70 &&
        fielder->nonCatchFlyBallStratInd != 0) {
    ...
    if (!g_d_GameSettings.minigamesEnabled) {
        ...
        } else if (resultCode == 1 && fielderIndex <= 5) {
            f32 lx = g_Ball.physicsSubstruct.futureCoordsAndDist[fielder->framesToGetToBallLandingSpot].pos.x;
            f32 lz = g_Ball.physicsSubstruct.futureCoordsAndDist[fielder->framesToGetToBallLandingSpot].pos.z;
...
void minigameFieldingRelated_collisions(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        for (j = i + 1; j < 9; j++) {
            ...
            if (outer->currentVelocity <= lbl_3_rodata_B20) {
                ...
                inner->pos.z = nz * (lbl_3_data_476C[outer->Weight] + lbl_3_data_476C[inner->Weight]) +
                               outer->pos.z;
            } else if (inner->currentVelocity <= lbl_3_rodata_B20) {
...
void fielding_handleCollisionsAndSpecialActions(void) {
    ...
    for (i = 0, fielder = g_Fielders; i < 9; i++, fielder++) {
        ...
        if ((g_GameLogic.gameStatus == GAME_STATUS_AT_BAT || g_GameLogic.gameStatus == GAME_STATUS_LIVE_BALL) &&
            fielder->distanceToMound > lbl_3_rodata_CA8 && g_Ball.totalFramesAtPlay > 1 &&
...
void updateFielder_SpecificValuesEachFrame(int fielderIndex) {
    ...
    if (fielderIndex == 0) {
        ...
        if ((g_Ball.framesSinceHit == fielder->lockoutDuration && g_GameLogic.gameStatus == GAME_STATUS_LIVE_BALL) ||
            ...
            pitcherShouldReset = !(g_Ball.lineDriveThroughPitcherInd != 0 && g_Ball.framesSinceHit < fielder->lockoutDuration);
            if (pitcherShouldReset && g_FieldingLogic.pitcher_inPitchingState != 0) {
    ...
    if (fielderIndex == 1) {
        if (!g_d_GameSettings.minigamesEnabled && g_FieldingLogic.catcherNotFocusedOnRunnerScoring != 0) {
            s16 framesSinceHit = g_Ball.framesSinceHit;

            if (framesSinceHit > g_Fielders[1].lockoutDuration && g_FieldingLogic.liveBallBcOfPickoffOrStealCd != 3) {
                g_FieldingLogic.catcherNotFocusedOnRunnerScoring = 0;
            } else {
                if (framesSinceHit <= 0x91 || g_FieldingLogic.liveBallBcOfPickoffOrStealCd != 3) {
                    ...
                    if (framesSinceHit >= 0x78) {
                        ...
                        if (g_Runners[3].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD &&
                            g_Runners[3].fractionalBasesRan >= lbl_3_rodata_CC0) {
                            g_FieldingLogic.catcherNotFocusedOnRunnerScoring = 0;
    ...
    dx = fielder->pos.x;
    dz = fielder->pos.z;
    fielder->distanceFromHomePlate = dolsqrtf2(dx * dx + dz * dz);

    for (i = 1; i <= 3; i++) {
        dx = base_MoundCoordinates[i].x - fielder->pos.x;
        dz = base_MoundCoordinates[i].z - fielder->pos.z;
        fielder->distanceToBases[i] = dolsqrtf2(dx * dx + dz * dz);
    }

    dx = base_MoundCoordinates[4].x - fielder->pos.x;
    dz = base_MoundCoordinates[4].z - fielder->pos.z;
    fielder->distanceToMound = dolsqrtf2(dx * dx + dz * dz);

    inWedge = TRUE;
    if (lbl_3_rodata_BD0 + (fielder->pos.x - fielder->pos.z) > lbl_3_rodata_B20 &&
        lbl_3_rodata_BD0 + (-fielder->pos.x - fielder->pos.z) > lbl_3_rodata_B20) {
        inWedge = FALSE;
    }

    if (!inWedge) {
        fielder->outFieldZoneCode = 0;
    } else {
        zoneDist = dolsqrtf2(fielder->pos.x * fielder->pos.x + fielder->pos.z * fielder->pos.z);
        if (zoneDist < lbl_3_rodata_C70) {
            fielder->outFieldZoneCode = 1;
        } else {
            dz = fielder->pos.z - lbl_3_rodata_C74;
            zoneDist = dolsqrtf2(fielder->pos.x * fielder->pos.x + dz * dz);
            if (zoneDist < lbl_3_rodata_C78) {
                fielder->outFieldZoneCode = 1;
            } else if (zoneDist < lbl_3_rodata_C7C) {
                fielder->outFieldZoneCode = 2;
            } else if (zoneDist < lbl_3_rodata_C80) {
                fielder->outFieldZoneCode = 3;
            } else {
                fielder->outFieldZoneCode = 4;
            }
        }
    }
    ...
    dx = g_Ball.AtBat_Contact_BallPos.x - fielder->pos.x;
    dz = g_Ball.AtBat_Contact_BallPos.z - fielder->pos.z;
    fielder->distanceFromBall = dolsqrtf2(dx * dx + dz * dz);

    if (g_Ball.AtBat_ContactResult == BALL_RESULT_TYPE_IN_AIR) {
        dx = g_Ball.landingSpotLocation.x - fielder->pos.x;
        dz = g_Ball.landingSpotLocation.z - fielder->pos.z;
        fielder->distanceFromLandingSpot = dolsqrtf2(dx * dx + dz * dz);
    }
    ...
    for (i = fielderIndex + 1; i < 9; i++) {
        ...
        fielder->distanceFromEachFielder[i] = dolsqrtf2(dx * dx + dz * dz);
    ...
    if (!g_d_GameSettings.minigamesEnabled) {
...

        {
            ...
            posZ = fielder->pos.z;
            if (posZ <= lbl_3_rodata_C18 && posZ >= lbl_3_rodata_C88) {
                posX = fielder->pos.x;
                if (posX <= lbl_3_rodata_B90 && posX >= lbl_3_rodata_CB8) {
                    if (posZ > base_MoundCoordinates[1].z) {
                        f32 lo = base_MoundCoordinates[2].z - lbl_3_rodata_B68;
                        f32 hi = lbl_3_rodata_B8C + base_MoundCoordinates[2].z;
    ...
    dx = fielder->IntendedLocation.x - fielder->pos.x;
    dz = fielder->IntendedLocation.z - fielder->pos.z;
    fielder->distanceFromAutoLocation = dolsqrtf2(dx * dx + dz * dz);
    ...
    if (g_Strikes.outs >= 3) {
        ...
        } else if (g_Ball.ballVelocity > lbl_3_rodata_BB8) {
            fielder->animationRelated = 2;
        }
...
void updateFielderMovementAndPosition(void) {
    ...
    if (g_Ball.pauseBallMovementWhenInPlant) {
        ...
        if (dist < fielderActionConstants[37]) {
            ...
            targetX = (fielderActionConstants[36] - lbl_3_rodata_B60) * (dx / dist) + fielder->pos.x;
            targetZ = (fielderActionConstants[36] - lbl_3_rodata_B60) * (dz / dist) + fielder->pos.z;
