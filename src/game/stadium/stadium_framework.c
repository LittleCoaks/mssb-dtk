...
typedef struct _StadiumLightParams {
    u8 lights[4][10];
    GXColor ambient;
} StadiumLightParams;
extern StadiumLightParams inputParams[14];
...
void loadStadiumLighting(int stadiumType, void* stadiumData) {
    ...
    if (stadiumData != NULL) {
        for (i = 0; i < 4; i++) {
            characterLightingRelated(inputParams[g_d_GameSettings._54].lights[i], &input);
            LITAlloc(&light);
...
void calledWhileMatchIsLoading(void) {
    if (lbl_803C6CF8[0x715] == 1) {
        ...
        loadStadiumLighting(g_d_GameSettings.StadiumID, hugeAnimStruct.stadiumFile);
...
void processStadiumObjectFunction(int stadiumId, int idx, int arg2, void* arg3) {
    if (idx < stadiumObjectCollision.objectCount) {
        if (stadiumObjectCollision.objects[idx].func != NULL) {
            stadiumObjectCollision.objects[idx].func(idx, arg2, arg3);
        }
    }
}
...
void fn_3_B8C08(Mtx view) {
    ...
    for (i = 0; i < stadiumObjectCollision.objectCount; i++) {
        ...
        if (obj->hasShadow && obj->model != NULL) {
            ...
            fn_800B2BFC(obj->model->root, obj->model->_64, obj->model->_66);
            fn_800B3F20(root);
            fn_800B313C(root);
...
void loadStadiumObjectVisuals(Mtx view, int flag, u32 pass) {
    ...
    ambient = inputParams[g_d_GameSettings._54].ambient;
    restore = ambient;
    input = lbl_80367318;
...
void fn_3_B867C(Mtx view, StadiumDrawOrder* order) {
    ...
    if (stadiumObjectCollision.gameStatusIsLiveBall) {
        count = 0;
        p = order;
        i = stadiumObjectCollision.objectCount;
        obj = &stadiumObjectCollision.objects[i - 1];
        i--;
        do {
            if (!obj->hasShadow) {
                p->depth = -512.0f;
