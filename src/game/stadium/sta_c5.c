...
void fn_3_EDFAC(void) {
    ...
    if (g_GameLogic.gameStatus != GAME_STATUS_MINIGAME_READY) {
        if (g_GameLogic.gameStatus == GAME_STATUS_PAUSED) {
            ...
            emitters[1] = initializeStadiumObjectEmitter(stadiumHazardSoundIDs[g_d_GameSettings.StadiumID] + 6, NULL, NULL, 4);
...
void fn_3_EE100(DKJungleDispObj* obj, Mtx view) {
    ...
    GXSetNumChans(1);
    scale = lbl_3_bss_AF04[0];
    PSMTXCopy(obj->mtx, model);
    PSMTXCopy(view, camMtx);
    camMtx[0][3] += lbl_3_data_1BA70[0];
    camMtx[1][3] += lbl_3_data_1BA70[1];
    scale = 1.0f + scale;
    camMtx[0][0] *= scale;
    camMtx[0][1] *= scale;
    camMtx[0][2] *= scale;
...
void fn_3_EE388(void) {
    ...
    bss->_24 = phase;
    if (fabs(phase) >= fabs(bss->_28)) {
        bss->_20 = bss->_20 * -1.0f;
    } else if ((int)(fabs(phase) / phase) != (int)(fabs(bss->_28) / bss->_28)) {
        water->needsReroll = 1;
    }
}
...
void fn_3_EE67C(DKJungleDispObj* obj, Mtx view) {
    ...
    for (i = 0; i < obj->drawList->count; i++, entry++) {
        ...
        case DKJUNGLE_ENTRY_SETUP_VTX:
            ...
            for (j = 0; j < 13; j++) {
                if (((entry->vtxDescBits >> (j * 2 + 2)) & 3) != 0) {
                    desc[n].mAttr = GX_VA_POS + j;
                    desc[n].mType = (entry->vtxDescBits >> (j * 2 + 2)) & 3;
                    n++;
                }
            }
...
void fn_3_EEB94(void) {
    ...
    for (y = 0; y < 16; y++) {
        for (x = 0; x < 16; x++) {
            ...
            a = *(u8*)(lbl_3_bss_B118[0] + idx);
            b = *(u8*)(lbl_3_bss_B118[0] + idx + 1);
            a += dir[idx] * (rand() % 14 + 8);
            ...
            *(u8*)(lbl_3_bss_B118[0] + idx) = a;
            *(u8*)(lbl_3_bss_B118[0] + idx + 1) = b;
        }
    }
}
...
void klaptrapHitAnimation(int idx) {
    ...
    if (obj->state < 3) {
        ...
        vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[0x12]
                                                                       : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x12];
        voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 9, vol, 0x3F, 0);
        val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD
                  ? lbl_3_data_84B8[0x13]
                  : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x13];
        ...
        case 2:
            if (obj->timer % tune->dustInterval[2] == 0) {
                ...
                vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD
                          ? lbl_3_data_84B8[0x10]
                          : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x10];
                voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 8, vol, 0x3F, 0);
                val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD
                          ? lbl_3_data_84B8[0x11]
                          : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x11];
                sndFXCtrl(voice, 0x5B, val);
            }
            break;
        }
...
void fn_3_EF3D4(StadiumObject* obj, u8 idx) {
    actorSetAnimBank((ACTActor*)obj->model, lbl_3_bss_B154[idx]);
}
    ...
    if (carrier != NULL) {
        if (carrier->ownerId != obj->_C3) {
            for (i = 0; i < 9; i++) {
                ...
                if (carrier != NULL && carrier->ownerId == obj->_C3) {
...
void dkKlaptrapChaseUpdate(DKJungleKlaptrap* obj) {
    ...
    if (dist < 1.25f) {
        if (fabs(v.y) <= 0.5) {
            ...
            for (i = 0; i < base[0x33C]; i++) {
                if (i != obj->index) {
                    DKJungleKlaptrap* other = (DKJungleKlaptrap*)&stadiumObjectCollision.objects[i + base[0x33B]];
            ...
            if (isWorldPosOnScreen((Vec*)&fielder->pos)) {
                stadiumID = g_d_GameSettings.StadiumID;
                vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD
                          ? lbl_3_data_84B8[0x14]
                          : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x14];
                voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 10, vol, 0x3F, 0);
                val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD
                          ? lbl_3_data_84B8[0x15]
                          : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x15];
                sndFXCtrl(voice, 0x5B, val);
            }
...
void dkKlaptrapRoamUpdate(DKJungleKlaptrap* obj) {
    ...
    for (i = 0; i < 9; i++) {
        ...
        if (fielder != NULL) {
            if (!isPosOutsideKlaptrapZone(*(Vec*)&fielder->pos, obj->kind) && fielder->jumpY <= 0.0f) {
                obj->_C1 = i;
                obj->state = 2;
                actorSetAnimBank((ACTActor*)obj->actor, lbl_3_bss_B154[1]);
                return;
            }
        }
    }
    ...
    case 1:
        if (obj->timer < 20) {
            obj->timer++;
            rad = 0.017453292f * -obj->rotY;
            next.x = obj->pos.x + (f32)(0.08 * (f32)cos(rad));
            next.y = obj->pos.y + 0.0f;
            next.z = obj->pos.z + (f32)(0.08 * (f32)sin(rad));
...
void maybeGharialCTRLRel(DKJungleKlaptrap* obj) {
    ...
    CTRLSetRotation((Control*)obj, 0.0f, obj->rotY, 0.0f);
    CTRLSetScale((Control*)obj, 1.0f, 1.0f, 1.0f);
    actorSetAnimBank((ACTActor*)obj->actor, lbl_3_bss_B154[0]);
...
void fn_3_F1674(void) {
    ...
    vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[0xA]
                                                                   : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0xA];
    voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 5, vol, 0x3F, 0);
    val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[0xB]
                                                                   : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0xB];
    sndFXCtrl(voice, 0x5B, val);
...
void fn_3_F22FC(DKJungleBarrel* barrel, s8 fielderIdx) {
    ...
    dir.y = 0.0f;
    PSVECNormalize(&dir, &dir);
    processFielderKnockout(idx, calculateAngleFromCoordinates(dir.x, dir.z));
...
s8 dkBarrel_collisionWithFielder(DKJungleBarrel* barrel) {
    ...
    for (i = 0; i < 7; i++) {
        ...
        offset.y *= -1.0f;
        range = offset.y < barrel->pos.y ? fielder->hitbox_barrelCollisions : 4.6666665f;
...
void fn_3_F2724(DKJungleBarrel* barrel, DKJungleKlaptrap* klaptrap) {
    ...
    if (absX <= 3.0 && absZ <= 1.75) {
        klaptrap->state = 5;
        stadiumID = g_d_GameSettings.StadiumID;
        vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[0x16]
                                                                       : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x16];
        voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 0xB, vol, 0x3F, 0);
        val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[0x17]
                                                                       : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 0x17];
        sndFXCtrl(voice, 0x5B, val);
    }
}
...
void dkBarrelAdvanceMotion(DKJungleBarrel* barrel) {
    ...
    t = 0.42f * bss->rate * (f32)bss->frame;
    oneMinusT = 1.0f - t;
    for (i = 0; i < 7; i++) {
        basis[i] = bss->factors[i] * pow(t, i) * pow(oneMinusT, 6 - i);
    }
    pt = barrel->pos;
    pt.z = pt.x = 0.0f;
    for (i = 0; i < 7; i++) {
        pt.x += basis[i] * bss->points[i].x;
        pt.z += basis[i] * bss->points[i].z;
    }
    pt.y += barrel->velY;
    ...
    if (pt.y < 0.0f) {
        ...
        if (barrel->velY > 0.022f) {
            stadiumID = g_d_GameSettings.StadiumID;
            vol = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[2]
                                                                           : stadiumHazardSoundFxRelated[stadiumID * 0x1E + 2];
            voice = sndFXStartEx(stadiumHazardSoundIDs[stadiumID] + 1, vol, 0x3F, 0);
            val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[3]
    ...
    if (barrel->pos.y < 0.5 && barrel->velY <= 0.0f) {
        for (i = 0; i < bss->numObjects; i++) {
            klaptrap = (DKJungleKlaptrap*)&stadiumObjectCollision.objects[bss->objectIndex + i];
            if (klaptrap->state < 3) {
...
void fn_3_F2FFC(DKJungleBarrel* barrel, f32 x, f32 z) {
    ...
    sign = (x - barrel->rotY) / fabs(x - barrel->rotY);
    if (sign == 0) {
...
void dkBarrelLaunch(DKJungleBarrel* barrel, f32 x, f32 z) {
    DKJungleBss* bss = (DKJungleBss*)lbl_3_bss_AEE0;
    f32* pts = (f32*)bss->points;
    Vec dir;
    s8 sign;
    DKJungleSmokeEmitter* emitter;
    DKJungleSmokeParticle* p;
    u8 i;
    f32 angle;
    ...
    dir.z = z - barrel->_B0;
    PSVECNormalize(&dir, &dir);
    bss->points[0].x = 10.0f * dir.x + barrel->rotY;
    pts[1] = 0.0f;
    pts[2] = 10.0f * dir.z + barrel->_B0;
    pts[3] = 0.33333334f * (x - barrel->rotY) + barrel->rotY;
    pts[4] = 0.0f;
    pts[5] = 0.33333334f * (z - barrel->_B0) + barrel->_B0;
    pts[6] = 0.6666667f * (x - barrel->rotY) + barrel->rotY;
    pts[7] = 0.0f;
    pts[8] = 0.6666667f * (z - barrel->_B0) + barrel->_B0;
    pts[9] = x;
    pts[10] = 0.0f;
    pts[11] = z;
    pts[12] = x + ((f32)(sign * 80) - x) / 3.0f;
    pts[13] = 0.0f;
    pts[14] = z;
    pts[15] = x + 2.0f * ((f32)(sign * 80) - x) / 3.0f;
    pts[16] = 0.0f;
    pts[17] = z;
    pts[18] = (f32)(sign * 80);
    pts[19] = 0.0f;
    ...
    if (emitter != NULL) {
        ...
        for (; p != NULL; p = p->next) {
            ...
            p->pos.z = pts[2] + (f64)(rand() % 600 - 300) / 100.0;
...
void dkJungleBarrelCannonCutsceneUpdate(DKJungleCannon* cannon) {
    ...
    v.x = bss->launchX - cannon->pos.x;
    v.y = 0.0f;
    v.z = tune->launchZ - cannon->pos.z;
    PSVECNormalize(&v, &v);
    ...
    if (g_GameLogic.gameStatus != GAME_STATUS_INNING_TRANSITION) {
        ...
        case 0:
            if (bss->_01 == 0) {
                dkBarrelLaunch(cannon->barrel, bss->launchX, tune->launchZ);
                fn_3_F4C4C(cannon);
                cannon->barrel->_C1 = 1;
                cannon->mode = 3;
                bss->_01 = 1;
                ...
                val = g_d_GameSettings.GameModeSelected == GAME_TYPE_TOY_FIELD ? lbl_3_data_84B8[1]
...
void fn_3_F4C4C(DKJungleCannon* cannon) {
    f32 angle = 0.017453292f * -cannon->rotY;

    cannon->vel.x = 3.0f * (f32)sin(angle);
    cannon->vel.y = 0.0f;
    cannon->vel.z = -(f32)cos(angle) * 3.0f;
...
void fn_3_F4DAC(void) {
    ...
    Vec dir;
    Vec forward = {0.0f, 0.0f, 1.0f};
    u8 accum = 100 / nBarrelLaunchers[0];
    u8 i = 0;
    u8 roll;
    f32 dist;
    f32 angle;
...
void processJungleObjectCollisions(Mtx view, struct _StadiumDrawOrder* order) {
    ...
    if (stadiumObjectCollision.gameStatusIsLiveBall) {
        ...
        for (i = 0; i < nBarrelLaunchers[0]; i++) {
...
void loadDKJungle(void** files) {
    ...
    *animTable = ActorObjectInitTable(propCount);
    ...
    bss->_67C = *animTable + sparkAnim * 0x90 + 0x34;
    lastAnim = ringAnim + 3;
    animateBallRelated(*animTable, lastAnim, lastAnim, files[ids[8]], 0, 0);
    lbl_3_bss_B154[0] = (u32)files[ids[16]];
    lbl_3_bss_B154[1] = (u32)files[ids[17]];
    ...
    o->fadeByDepth = 1;
    o->alpha = 0xFF;
    o->effect = (StadiumObjectEffect*)&lbl_3_bss_B154[3];
    o->preDraw = NULL;
    o->animActive = 0;
    o->animIndex = -1;
    o->drawPass = 1;
    ...
    o[1].fadeByDepth = 1;
    o[1].alpha = 0xFF;
    o[1].effect = (StadiumObjectEffect*)&lbl_3_bss_B154[26];
    ...
    o += 3;
    b = (DKJungleBarrel*)o;
    for (; i < stadiumObjectCollision.objectCount; i++) {
        b->_9D[0] = 0;
        o->model = NULL;
        o->triangles = NULL;
        o->callback = NULL;
