...
extern const f32 lbl_3_rodata_12C8;
extern const f32 lbl_3_rodata_12CC;
...
extern void setPitcherStatsToInMemPitcher(int rosterIdx);
extern int loadCharacterAnimation(int charID);
...
BOOL waitingForPitch_checkForPickoffs(void) {
    ...
    do {
        if (g_Pitcher.AIInd != 0) {
            ...
            } else if (AI_getPort(g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]) !=
                       0) {
                controls = &g_Minigame._1D7C[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
            } else if (g_d_GameSettings.minigamesEnabled && (s8)g_Minigame.minigamePlayerSelectedOrder >= 0) {
                controls = &g_Controls[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
            }
...
void fn_3_6FA28(void) {
    ...
    if (g_Ball.AtBat_Contact_BallPos.y < g_Ball.groundYForBounces) {
        ...
        g_Ball.physicsSubstruct.velocity.y =
            g_Ball.physicsSubstruct.velocity.y *
            (ballBounceConstants[stadium].yOtherBouncesSpeedKept - g_Ball.physicsSubstruct.velocity.y * lbl_3_rodata_1250);
        g_Ball.physicsSubstruct.velocity.y = -g_Ball.physicsSubstruct.velocity.y;
        if (g_Ball.physicsSubstruct.velocity.y < lbl_3_rodata_1254) {
...
void fn_3_6FDA0(void) {
    ...
    scale = lbl_3_rodata_1254 * (f32)(g_Ball.StaticRandomInt2 % 10);
    speed = dolsqrtf2(g_Pitcher.ballVelocity.x * g_Pitcher.ballVelocity.x +
                      g_Pitcher.ballVelocity.z * g_Pitcher.ballVelocity.z);
    g_Ball.physicsSubstruct.velocity.x = (lbl_3_rodata_1278 + scale) * (dirX * speed);
    g_Ball.physicsSubstruct.velocity.z = (lbl_3_rodata_1278 + scale) * (dirZ * speed);
    g_Ball.physicsSubstruct.velocity.y = lbl_3_rodata_1258;
}
...
void fn_3_70280(void) {
    ...
    dx = (int)(lbl_3_rodata_1288 * (g_Pitcher.ballCurrentPosition.x - g_Batter.batterPos.x));
    dz = (int)(lbl_3_rodata_1288 * (g_Pitcher.ballCurrentPosition.z - g_Batter.batterPos.z));
    if (g_Batter.batterHand != BATTING_HAND_RIGHT) {
        dx = -dx;
    }
    idx = characterStaticIndexes[g_Batter.charID * 6 + 2] * 3;
    if (dz <= hitByPitchHitboxes[idx] && (f32)dz >= lbl_3_rodata_1258 && dx <= hitByPitchHitboxes[idx + 1] &&
...
void fn_3_70838(void) {
    ...
    if (g_Pitcher.eggBallBounceNumber == 0) {
        ...
        g_Pitcher.frontOfPlateZ = lbl_3_rodata_1250 * (g_Pitcher.eggBallBounceZLoc + g_Pitcher.frontOfPlateZ);
    } else {
...
void fn_3_709B4(void) {
    ...
    if (g_Pitcher.bulletPitchFrameCounter >= g_Pitcher.bulletPitchLoopFrames) {
        ...
        angle = angle * lbl_3_rodata_12A0;
        angle = angle + lbl_3_rodata_12A4;
        getComponentsFromRad(angle, &sinA, &cosA);
        k = lbl_3_rodata_1254 * (f32)bulletBallConstants[g_Pitcher.starPitchType * 5 - 0x1F];
        g_Pitcher.starPitchPositionAdjustment.x = lbl_3_rodata_1258;
...
void fn_3_70AEC(void) {
    if (g_Pitcher.cancelParabolicAdjustmentInd != 0) {
        g_Pitcher.pitchX_parabolicAdjustment = lbl_3_rodata_1258;
    } else {
        f32 t = g_Ball.pitchHangtimeCounter - g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250;
        g_Pitcher.pitchX_parabolicAdjustment =
            g_Pitcher.horizontalOffsetParabolaMidpoint - t * (g_Pitcher.horizontalGlobalParabolicVelo * t) / lbl_3_rodata_12A8;
    }
}

extern const f32 lbl_3_rodata_12AC;
...
void fn_3_70B94(void) {
    ...
    if (g_Pitcher.starPitchType == 9 || g_Pitcher.starPitchType == 10) {
        if (g_Pitcher.eggBallBounceNumber == 0) {
            t = (f32)g_Ball.pitchHangtimeCounter - (f32)g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250;
            g_Pitcher.pitchY_parabolicAdjustment =
                g_Pitcher.verticalOffsetParabolaMidpoint -
                t * (g_Pitcher.verticalGlobalParabolicVelo * t) / lbl_3_rodata_12A8;
            ...
            if (g_Pitcher.framesSinceFirstEggBounce == 1) {
                if (g_Pitcher.eggBallBounceNumber == 1) {
                    ...
                    g_Pitcher.ballBouncePeakZ = (s16)((f32)frames * lbl_3_rodata_1250);
                    ...
                    g_Pitcher.verticalOffsetParabolaMidpoint = lbl_3_rodata_12AC + strikeZone[4];
                    t = (f32)g_Pitcher.framesUntilBallReachesBatterZ;
                    g_Pitcher.ballBouncePeakZ = g_Pitcher.framesUntilBallReachesBatterZ;
                    g_Pitcher.verticalGlobalParabolicVelo =
                        lbl_3_rodata_12A8 * g_Pitcher.verticalOffsetParabolaMidpoint / (t * t);
                }
            }
            ...
            g_Pitcher.pitchY_parabolicAdjustment = lbl_3_rodata_1258;
        }
...
void pitchCurve(void) {
    ...
    g_Pitcher.pitchCurveVeloV1 = lbl_3_rodata_1258;
    g_Pitcher._60 = lbl_3_rodata_1258;
    if (g_Pitcher.cancelParabolicAdjustmentInd == 0 && g_Pitcher.calced_curve != 0) {
        curve = LinearInterpolateToNewRange((f32)g_Pitcher.calced_curve, lbl_3_rodata_1290, lbl_3_rodata_1288,
                                            (f32)pc[1],
                                            (f32)pc[2]);
        curve = lbl_3_rodata_12B0 * curve;
        if (g_Pitcher.AIInd != 0) {
        ...
        frames = LinearInterpolateToNewRange((f32)g_Pitcher.calced_curveControl, lbl_3_rodata_1290, lbl_3_rodata_1288,
                                             curveControlFrames_CursedBallMult[0], curveControlFrames_CursedBallMult[1]);
        if (frames < lbl_3_rodata_1290) {
            frames = lbl_3_rodata_1290;
        }
        if (dir == 0) {
            f32 step = curve / frames;
            if (g_Pitcher.pitchCurveVeloV2 < lbl_3_rodata_1258) {
                g_Pitcher.pitchCurveVeloV2 += step;
                if (g_Pitcher.pitchCurveVeloV2 > lbl_3_rodata_1258) {
                    g_Pitcher.pitchCurveVeloV2 = lbl_3_rodata_1258;
                }
            } else {
                g_Pitcher.pitchCurveVeloV2 -= step;
                if (g_Pitcher.pitchCurveVeloV2 < lbl_3_rodata_1258) {
...
extern const f32 lbl_3_rodata_12B4;
extern const f32 lbl_3_rodata_12B8;
extern const f32 lbl_3_rodata_12BC;
extern const f32 lbl_3_rodata_12C0;
extern const f32 lbl_3_rodata_12C4;
    ...
    if (g_Minigame.GameMode_MiniGame == MINI_GAME_ID_WALLBALL && g_Minigame.ballStoppedBreakingWallsInd) {
        ...
        do {
            ...
            if (g_Pitcher.ballCurrentPosition.z <= g_Pitcher.pitchZ_whenAirResistanceStarts) {
                ...
                if (d < lbl_3_rodata_128C) {
                    g_Pitcher.ballVelocity.x = g_Pitcher.ballVelocity.x - g_Pitcher.ballVelocity.x * g_Pitcher.airResistance_veloAdj;
                    g_Pitcher.ballVelocity.y = g_Pitcher.ballVelocity.y - g_Pitcher.ballVelocity.y * g_Pitcher.airResistance_veloAdj;
        ...
        if (g_Pitcher.starPitchType == 3) {
            ...
            if (d >= lbl_3_rodata_1258) {
                g_Pitcher.starPitchPositionAdjustment.x = lbl_3_data_5EB8[0] * (d / lbl_3_data_5EB8[1]);
            ...
            if (d >= lbl_3_rodata_1258) {
                ...
                if (d > lbl_3_rodata_1290) {
                    d = lbl_3_rodata_1290;
                }
                sinVal = SINF(lbl_3_rodata_12B4 * d);
                g_Pitcher.starPitchPositionAdjustment.x = lbl_3_rodata_12B8 * amp * -sinVal + -amp;
                if (g_Pitcher.warioWaluStarPitchRightLeft != 0) {
                    g_Pitcher.starPitchPositionAdjustment.x = -g_Pitcher.starPitchPositionAdjustment.x;
                }
                g_Ball.AtBat_Contact_BallPos.x += g_Pitcher.starPitchPositionAdjustment.x;
        ...
        if (g_Pitcher.starPitchType == 11 || g_Pitcher.starPitchType == 12) {
            idx = g_Pitcher.starPitchType - 11;
        ...
        if (g_Ball.AtBat_Contact_BallPos.y < ground) {
            if ((g_Pitcher.starPitchType == 9 || g_Pitcher.starPitchType == 10) &&
                ...
                speed = dolsqrtf2(g_Pitcher.ballVelocity.x * g_Pitcher.ballVelocity.x +
                                  g_Pitcher.ballVelocity.z * g_Pitcher.ballVelocity.z);
                if (speed < lbl_3_rodata_1254) {
                    speed = lbl_3_rodata_1254;
                }
                g_Pitcher.pitchCurveVeloV1 = lbl_3_rodata_1258;
                g_Pitcher._60 = lbl_3_rodata_1258;
                g_Pitcher.ballVelocity.x = lbl_3_rodata_12BC * (g_Pitcher.ballVelocity.x / speed);
                g_Pitcher.ballVelocity.z = lbl_3_rodata_12BC * (g_Pitcher.ballVelocity.z / speed);
        ...
        if (g_Ball.AtBat_Contact_BallPos.z < lbl_3_rodata_12C0) {
...
void pitchSetPhysicsConstants(void) {
    ...
    if (g_d_GameSettings.GameModeSelected == GAME_TYPE_PRACTICE && g_Practice.instructionNumber >= 0) {
        controls = &g_Practice.inputs[g_GameLogic.teamFielding];
    } else if (AI_getPort(g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]) != 0) {
        controls = &g_Minigame._1D7C[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
    } else if (g_d_GameSettings.minigamesEnabled && (s8)g_Minigame.minigamePlayerSelectedOrder >= 0) {
        controls = &g_Controls[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
    }
    pc = pitchConstantsArray[g_Pitcher.specialPitchTypeCode];
    veloZ = -((f32)g_Pitcher.pitchSpeed / g_Pitcher.pitchSpeedScaler);
    g_Pitcher.ballVelocity.z = veloZ;
    g_Pitcher.moundZ = lbl_3_rodata_12C8;
    g_Pitcher.pitchZ_whenAirResistanceStarts = (lbl_3_rodata_12C8 * (f32)(100 - pc[4])) / lbl_3_rodata_1288;
    ballZ = g_Pitcher.ballCurrentPosition.z;
    dz = ballZ - g_Pitcher.frontOfPlateZ;
    g_Pitcher.ballVelocity.x = -((g_Pitcher.pitchStartingPosition_AIMaxCurve - g_Pitcher.ballCurrentPosition.x) * veloZ / dz);
    g_Pitcher.ballVelocity.y = -((g_Pitcher.eggBallBounceYHeight - g_Pitcher.ballCurrentPosition.y) * veloZ / dz);
    g_Pitcher.airResistance_veloAdj = lbl_3_rodata_12CC * (f32)pc[3];
    ...
    g_Pitcher.verticalOffsetParabolaMidpoint =
        ((f32)g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250) *
        (g_Pitcher.verticalGlobalParabolicVelo * ((f32)g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250)) / lbl_3_rodata_12A8;
    ...
    g_Pitcher.horizontalOffsetParabolaMidpoint =
        ((f32)g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250) *
        (g_Pitcher.horizontalGlobalParabolicVelo * ((f32)g_Pitcher.frameWhenUnhittable * lbl_3_rodata_1250)) / lbl_3_rodata_12A8;
    ...
    if (g_Pitcher.starPitchType == 7 || g_Pitcher.starPitchType == 8) {
        int bi = (g_Pitcher.starPitchType - 7) * 5;
    ...
    if (g_Pitcher.starPitchType != 0) {
        ...
        if (g_Pitcher.ChargePitchType == 3 || g_Pitcher.nonCaptainStarPitchTriggeredType == 2) {
            ...
            g_Pitcher.pitchSpeed = g_Pitcher.fastBallSpeed -
                                   (int)(lbl_3_data_5F08[9] * ((f32)g_Pitcher.fastBallSpeed - (f32)g_Pitcher.curveBallSpeed) *
                                         (lbl_3_rodata_1290 - g_Pitcher.pitchChargeUp));
        } else {
        ...
        if (g_Pitcher.nonCaptainStarPitchTriggeredType == 0 &&
            ...
            if (g_Pitcher.ChargePitchType != 0) {
                g_Pitcher.pitchSpeed = (f32)g_Pitcher.pitchSpeed * -(lbl_3_rodata_1254 * (f32)lbl_3_data_5EDC[6] - lbl_3_rodata_1290);
            } else if (g_Pitcher.TypeOfPitch != 2) {
                curve = curve * -(lbl_3_rodata_1254 * (f32)lbl_3_data_5EDC[5] - lbl_3_rodata_1290);
            }
            curveControl = curveControl * -(lbl_3_rodata_1254 * (f32)lbl_3_data_5EDC[7] - lbl_3_rodata_1290);
            cursedBall = cursedBall * -(lbl_3_rodata_1254 * (f32)lbl_3_data_5EDC[8] - lbl_3_rodata_1290);
        }
...
void pitchBeingReleased(void) {
    ...
    if (g_Minigame.GameMode_MiniGame == MINI_GAME_ID_BOBOMB_DERBY || g_Minigame.GameMode_MiniGame == MINI_GAME_ID_BARREL_BATTER ||
        ...
        if (g_d_GameSettings.minigamesEnabled) {
            character = g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder];
        }
    ...
    g_Pitcher.ballCurrentPosition.x = ballCoords.x;
    g_Pitcher.pitchRelease.x = ballCoords.x;
    g_Pitcher.ballCurrentPosition.y = -ballCoords.y;
    g_Pitcher.pitchRelease.y = -ballCoords.y;
    g_Pitcher.ballCurrentPosition.z = ballCoords.z;
    g_Pitcher.pitchRelease.z = ballCoords.z;
    if (g_Pitcher.nonCaptainStarPitchTriggeredType != 0) {
    ...
    if (g_Pitcher.AIInd == 0 && (g_Pitcher.ChargePitchType == 3 || g_Pitcher.nonCaptainStarPitchTriggeredType != 0 ||
        ...
        if (g_d_GameSettings.minigamesEnabled) {
            setCharacterAnimations(g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder], 2);
        } else {
    ...
    if (frame == transitionFrames - 0xB) {
        ...
        if (g_Pitcher.strikeOutOrWalk == 1) {
            ...
            if (g_Pitcher.strikeOutOrWalk == 3) {
                ...
                for (i = 1; i < 4; i++) {
                    if (g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_ON_FIELD ||
                        g_Runners[i].runnerOnFieldOrOutOrScored == RUNNER_STATUS_WALK_WHILE_STEALING) {
                        if (!chainBroken) {
...
void fn_3_73FAC(void) {
    ...
    if (g_Pitcher.starPitchType == CAPTAIN_STAR_TYPE_WARIO || g_Pitcher.starPitchType == CAPTAIN_STAR_TYPE_WALUIGI) {
        g_Pitcher.pitchStartingPosition_AIMaxCurve = lbl_3_rodata_1258;
...
void pitchingWindUpFunction(void) {
    ...
    if (g_Pitcher.pitchTotalTimeCounter == 1) {
        ...
        g_Pitcher.pitchChargeUp = lbl_3_rodata_1258;
        g_Pitcher.pitchChargeUpAnimationProportion = lbl_3_rodata_1258;
    }
    if (g_Pitcher.AIInd != 0) {
        if (g_AiLogic.aIPitchType == 1 || g_AiLogic.aIPitchType == 3) {
            ...
            g_Pitcher.pitchChargeUp = lbl_3_rodata_1290;
            g_Pitcher.unknownFrameCounter++;
            g_Pitcher.pitchChargeUpAnimationProportion = lbl_3_rodata_1290 - (f32)(g_Pitcher.windupCountdownUntilBallReleased - 1) /
                                                                                 (f32)g_Pitcher.pitchWindUpCountDown;
            if (g_Pitcher.pitchChargeUpAnimationProportion > lbl_3_rodata_1290) {
                g_Pitcher.pitchChargeUpAnimationProportion = lbl_3_rodata_1290;
            }
        ...
        if (g_Pitcher.ChargePitchType == 1 || g_Pitcher.starPitchInd != 0 || g_Pitcher.ballHaloTrainInd_unused != 0) {
            ...
            if (g_Pitcher.ChargePitchType != 0) {
                g_Pitcher.pitchChargeUp = lbl_3_rodata_1290 - (f32)(g_Pitcher.windupCountdownUntilBallReleased - 1) /
                                                                  (f32)g_Pitcher.pitchWindUpCountDown;
                if (g_Pitcher.pitchChargeUp > lbl_3_rodata_1290) {
                    g_Pitcher.pitchChargeUp = lbl_3_rodata_1290;
                }
            } else {
                g_Pitcher.pitchChargeUp = lbl_3_rodata_1258;
...
void pitcherMovementOnMound(void) {
    ...
    if (g_Pitcher.pitcherOffCenter != 0) {
        if (g_Pitcher.pitcher.x >= -lbl_3_data_4474[3] && g_Pitcher.pitcher.x <= lbl_3_data_4474[3]) {
            ...
            g_Pitcher.pitcher.x = lbl_3_rodata_1258;
        } else if (g_Pitcher.pitcher.x < lbl_3_rodata_1258) {
...
void waitingForPitch(void) {
    ...
    if (g_d_GameSettings.minigamesEnabled) {
        ...
        controls = &g_Controls[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
        if (AI_getPort(g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]) != 0) {
            controls = &g_Minigame._1D7C[g_Minigame.minigameControlStruct[0].characterIndex[(s8)g_Minigame.minigamePlayerSelectedOrder]];
        }
        if (g_Pitcher.AIInd != 0) {
...
void setDefaultInMemPitcher(void) {
    ...
    g_Pitcher.centerOfStrikeZone.x = lbl_3_rodata_1250 * (strikeZone[0] + strikeZone[1]);
    g_Pitcher.centerOfStrikeZone.z = lbl_3_rodata_1250 * (strikeZone[2] + strikeZone[3]);
    ...
    g_Pitcher.ballCurrentPosition.y = lbl_3_rodata_1290;
    g_Pitcher.ballCurrentPosition.z = fieldingStartingCoords_regular[0].z;
    g_Pitcher.ballVelocity.x = lbl_3_rodata_1258;
    g_Pitcher.ballVelocity.y = lbl_3_rodata_1258;
    g_Pitcher.ballVelocity.z = lbl_3_rodata_1258;
    ...
    g_Pitcher.starPitchPositionAdjustment.x = lbl_3_rodata_1258;
    g_Pitcher.starPitchPositionAdjustment.y = lbl_3_rodata_1258;
    g_Pitcher.starPitchPositionAdjustment.z = lbl_3_rodata_1258;
    g_Pitcher.pitchWindUpCountDown = lbl_3_data_8080[1];
    g_Pitcher.curvePitchWindupFrames = lbl_3_data_8080[2];
    g_Pitcher.windupCountdownUntilBallReleased = lbl_3_data_8080[1];
    if (g_d_GameSettings.GameModeSelected == GAME_TYPE_PRACTICE) {
        ...
        g_Pitcher.AIInd =
            g_Minigame.minigameControlStruct[0].battingHandedness[(s8)g_Minigame.minigamePlayerSelectedOrder];
    } else {
...
void atBat_Pitcher(void) {
    ...
    case PITCHER_ACTION_STATE_HIT_BY_PITCH:
        ...
        g_Ball.pastCoordinates[0].y = g_Ball.AtBat_Contact_BallPos.y;
        g_Ball.pastCoordinates[0].z = g_Ball.AtBat_Contact_BallPos.z;
        g_Ball.physicsSubstruct.velocity.y = g_Ball.physicsSubstruct.velocity.y - g_Ball.physicsSubstruct.gravity;
        adjustVeloByAirResistance();
        g_Ball.AtBat_Contact_BallPos.x = g_Ball.AtBat_Contact_BallPos.x + g_Ball.physicsSubstruct.velocity.x;
        g_Ball.AtBat_Contact_BallPos.y = g_Ball.AtBat_Contact_BallPos.y + g_Ball.physicsSubstruct.velocity.y;
        g_Ball.AtBat_Contact_BallPos.z = g_Ball.AtBat_Contact_BallPos.z + g_Ball.physicsSubstruct.velocity.z;
        if (g_Ball.AtBat_Contact_BallPos.y < g_Ball.groundYForBounces) {
            g_Ball.physicsSubstruct.velocity.y =
                g_Ball.physicsSubstruct.velocity.y *
                (ballBounceConstants[g_d_GameSettings.StadiumID].yOtherBouncesSpeedKept -
                 g_Ball.physicsSubstruct.velocity.y * lbl_3_rodata_1250);
            g_Ball.physicsSubstruct.velocity.y = -g_Ball.physicsSubstruct.velocity.y;
            if (g_Ball.physicsSubstruct.velocity.y < lbl_3_rodata_1254) {
