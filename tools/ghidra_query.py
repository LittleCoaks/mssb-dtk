...
# game REL .text base in RAM: mapped_addr - .text offset (constant across the REL)
GAME_REL_TEXT_BASE = 0x8063F094
...
def config_symbols():
    """{name: ram addr} from config/*/symbols.txt (REL addresses are rebased)."""
    out = {}
    for p in (REPO / "config").rglob("symbols.txt"):
        is_rel = "game" in p.parent.name
        for line in p.read_text(encoding="utf-8", errors="replace").splitlines():
            m = re.match(r"^\s*(\w+)\s*=\s*\.?[\w.]*:0x([0-9A-Fa-f]+)", line)
            if m:
                a = int(m.group(2), 16)
                if is_rel and a < 0x80000000:
                    a += GAME_REL_TEXT_BASE
